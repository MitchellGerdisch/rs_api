...
if [ $# -ne 4 ] 
then
	echo "USAGE: $0 API_ENDPOINT REFRESH_TOKEN CLOUD_ID SSH_KEY_NAME"  
	...
	echo "Where SSH_KEY_NAME is the name of the SSH key you want to create."
	exit 1
fi
...
curl --verbose --include \
     ...
     -X POST "${base_uri}/api/clouds/${cloud_id}/ssh_keys" \
	-d "name=${key_name}"
